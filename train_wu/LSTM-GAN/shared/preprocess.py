...
Output:
  processed/all_data.parquet (default)
"""
import argparse
import os
import sys
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="合并 daily + metric CSV 为单个 Parquet 文件")
    parser.add_argument("--data_dir", type=str, default=DATA_DIR,
                        help="原始数据目录路径 (默认: documents-export-2026-5-15)")
    parser.add_argument("--output", type=str, default=OUTPUT_PATH,
                        help="输出 Parquet 文件路径 (默认: processed/all_data.parquet)")
    return parser.parse_args(argv)


def load_basic_filter(data_dir):
...
def main(argv=None):
    ...
    # Update global paths used in helper functions
    global DATA_DIR, OUTPUT_PATH
    DATA_DIR = data_dir
    OUTPUT_PATH = output_path

    # 1. Filter allowed stocks
    allowed_stocks = load_basic_filter(data_dir)
