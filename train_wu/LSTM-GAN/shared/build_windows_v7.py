"""build_windows_v7.py — 完整因子提取流水线（26 维，T+1 标签）

从 all_data.parquet 构建滑动窗口 .pt 文件，包含完整因子工程：
  - 8 个技术指标（MACD, RSI, BB, 动量, 波动率）
  - VWAP gap（close/vwap - 1）
  - 对数变换（vol, amount, total_mv）
  - PE/PB 截断排名 [0.1, 500] / [0.1, 50]
  - Winsorization 截断到 [P1, P99]
  - 两阶段标准化：时序 Z-score → 截面 Z-score
  - T+1 标签（单日下个交易日收益）

Features (26 维):
  Temporal (19): 10 raw + 1 vwap_gap + 8 tech（按股票在 T=60 上 Z-score）
  Cross    (7) : 3 rank + rel_beta + 3 val rank（按日期在 N 个股票间 Z-score 后 tile）

输入: processed/all_data.parquet
输出: processed/windows_v7/{train,val}/{year}/{date}.pt
"""
...
DATA_DIR = os.path.join(ROOT, "documents-export-2026-5-15")
INDEX_PATH = os.path.join(DATA_DIR, "market", "000300.SH.csv")
WINDOWS_DIR = os.path.join(ROOT, "processed", "windows_v7")
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="构建 v7 滑动窗口 .pt 文件（26 维因子）")
    parser.add_argument("--parquet", type=str, default=PARQUET_PATH,
                        help="输入的 Parquet 文件路径 (默认: processed/all_data.parquet)")
    parser.add_argument("--data_dir", type=str, default=DATA_DIR,
                        help="原始数据目录, 用于 market/index 文件 (默认: documents-export-2026-5-15)")
    parser.add_argument("--output_dir", type=str, default=WINDOWS_DIR,
                        help="输出目录 (默认: processed/windows_v7)")
...
def main(argv=None):
    ...
    global PARQUET_PATH, DATA_DIR, INDEX_PATH, WINDOWS_DIR
    PARQUET_PATH = args.parquet
    DATA_DIR = args.data_dir
    INDEX_PATH = os.path.join(DATA_DIR, "market", "000300.SH.csv")
    WINDOWS_DIR = args.output_dir

    os.makedirs(WINDOWS_DIR, exist_ok=True)

    print("[v7 build] Loading parquet ...")
    df = pd.read_parquet(PARQUET_PATH)
    df["trade_date"] = df["trade_date"].astype(str)
    for col in BASIC_RAW + TECH + VAL_COLS + ["vwap"]:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors="coerce")
    print(f"[v7 build] {len(df)} rows, {df['ts_code'].nunique()} stocks")

    print("[v7 build] Loading CSI300 index ...")
    ...
    print(f"[v7 build] T={T}, label=T+{LABEL_HORIZON}, log_cols={LOG_COLS}")
    ...
    print(f"[v7 build] Done. {len(all_pt)} files in {WINDOWS_DIR}")
