"""Quick sweep on v7 Spatial scores (already precomputed)."""
...
print(f"Scores: {len(scores):,} rows, {len(dates_s)} dates")
...
print(f"\n{'='*60}")
print(" v7 Spatial T+1 Sweep (Strategy B)")
print(f"{'='*60}")
...
for N in [5,10,15,20,25,30]:
    for K in [2,3,5,8,10]:
        ...
        if daily_p:
            ...
            results.append({"N":N,"K":K,"cum":round(cum,4),"excess":round(cum-cum_c,4),"days":len(daily_p)})
...
res.to_csv(r"D:\Workspace\DL_HW\deep-learning2026final-lab\train_li\v7\results\sweep_spatial.csv", index=False)
