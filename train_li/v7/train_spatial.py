"""v7/train_spatial.py — Train GRU + Spatial Concat on T+1 labels.

Config: H=128, L=1, D=0.2, lr=3e-4, K=5, d=32, N=1024, val_every=2, PATIENCE=7.
"""
...
WINDOW_SIZE = 60; EPOCHS = 50; PATIENCE = 7; GRAD_CLIP = 1.0
SCHEDULER_FACTOR = 0.5; SCHEDULER_PATIENCE = 5

CKPT_DIR = os.path.join(SCRIPT_DIR, "checkpoints")
RESULTS_DIR = os.path.join(SCRIPT_DIR, "results")
RESULTS_CSV = os.path.join(RESULTS_DIR, "results_spatial.csv")
os.makedirs(CKPT_DIR, exist_ok=True); os.makedirs(RESULTS_DIR, exist_ok=True)

def train_one(args, device):
    name = f"gru_spatial_v7_d{args.d_proj}_K{args.K}_H{args.hidden}_L{args.layers}_D{args.dropout}_lr{args.lr}_N{args.n_sample}"
    ckpt_path = os.path.join(CKPT_DIR, f"{name}.pt")

    print(f"\n{'='*60}")
    print(f" v7 Spatial T+1: {name}")
    print(f"  d_proj={args.d_proj} K={args.K} n_sample={args.n_sample} val_every={args.val_every}")
    print(f"{'='*60}")

    print("[v7-spatial] Loading dataset ...")
    ...
    model = GRURankerSpatial(input_dim=args.input_dim, hidden_size=args.hidden,
                              num_layers=args.layers, dropout=args.dropout,
                              d_proj=args.d_proj, K=args.K).to(device)

    params = sum(p.numel() for p in model.parameters())
    print(f"[v7-spatial] Params: {params:,}")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if no_imp >= PATIENCE:
            print(f"  [v7-spatial] Early stop @ epoch {epoch}"); break

    elapsed = time.time() - t0; avg_ep = np.mean(epoch_times) if epoch_times else 0

    result = {
        "name": name, "input_dim": args.input_dim, "d_proj": args.d_proj, "K": args.K,
    ...
    print(f"\n[v7-spatial] DONE: {name} -> IC={best_ic:.4f} @ ep{best_ep} ({elapsed/60:.0f}min)")
...
def main():
    ...
    print(f"[v7-spatial] device={device}")
