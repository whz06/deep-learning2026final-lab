...
路线B的CNN-LSTM以收盘价回归（MSE）为训练目标，模型输出为绝对价格预测值
$\hat{p}_{t+1}$，再通过$\hat{r} = \hat{p}_{t+1} / p_t - 1$反算预期收益率进行排序。
这一范式与路线A（直接优化排序的ListMLE）存在根本差异。

在统一回测框架下对路线B执行N-K轮动时，发现由于\textbf{模型训练目标与回测使用方式的范式错位}，
回测结果无法可靠地反映模型的真实排序能力。具体而言：

\begin{itemize}[leftmargin=*]
    \item 模型从未以优化横截面排序为目标进行训练，其MSE损失优化的是价格预测精度，
          而非收益率排序的准确性。
    \item 将价格预测值转化为收益率的过程中，低价股因基数小而获得不成比例的高收益率权重，
          导致Top N持仓被低价高波动股（尤其是科创板股票）主导。
    \item 回测采用复利计算（\texttt{cumprod}），前期的选股偏差经过复利累积后被大幅放大。
\end{itemize}

因此，路线B的N-K轮动回测结果在此不予列示，以避免误导性结论。
路线B的评估以其\textbf{信号质量指标}（Rank IC、ICIR、方向胜率）为准，
这些指标不依赖策略参数，能够更诚实地反映模型对股票收益率的排序能力。
路线A各模型的回测结果基于ListMLE排序损失训练，训练目标与回测使用方式一致，
其回测结果具有直接的参考意义。
...
图\ref{fig:ch5_equity}展示了路线A三个模型在统一回测框架下的累计收益曲线（路线B的结果因模型范式差异未纳入回测比较，详见第四章说明）。
...
但路线B在统一框架下的累计收益与路线A不具有直接可比性（详见第四章的讨论）。
...
图\ref{fig:ch5_bars}以柱状图形式汇总了路线A三个模型在五个关键维度上的表现。
需要特别注意的是，各模型的IC统计口径一致（均为Rank IC），
ICIR、累计收益率和最大回撤基于同一回测框架，可以公平比较。
路线B因模型范式差异（价格回归vs排序学习），其回测结果不适用于此比较框架。

V7 GRU在累计收益率上表现最优（+10.0\%），V7 Spatial在Rank IC和ICIR上领先。
所有模型的最大回撤均在-14\%至-16\%之间——N=5的集中持仓模式天然伴随较高的回撤风险。
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{路线B的Pearson IC（0.118）显著高于路线A的Rank IC（0.069--0.074），
          但策略收益的回测值高度依赖于成本模型和策略参数}。回归模型的IC不能直接等价于排序策略的收益。
...
% ═══════════════════════════════════════════════════════════
\section{失败的补充尝试：Factor-GAN}
% ═══════════════════════════════════════════════════════════

在两条主路线之外，吴同学还尝试了一个基于生成对抗网络的补充方案——
Factor-GAN，将收益率预测建模为一个生成对抗问题。
该尝试未能在竞赛前产出可用结果，但作为方法论探索在此记录。

\subsection{设计动机}

传统回归模型（如路线B的CNN-LSTM）使用MSE损失最小化预测误差，
但MSE对收益率分布的两个特性处理不佳：
\begin{itemize}[leftmargin=*]
    \item \textbf{分布的双峰性}：A股日收益率在0附近高度集中（约70\%的交易日$|r|<$ 2\%），
          但尾部存在$\pm$10\%（主板）乃至$\pm$20\%（科创板）的极端值。
          MSE对尾部极端值的敏感度过高，导致模型倾向于预测接近均值的保守值。
    \item \textbf{信号-噪声比极低}：单日收益率的噪声远大于信号，
          模型很难区分真实的预测信号和运气好匹配上的噪声。
\end{itemize}

Factor-GAN的思路是：让生成器G负责预测收益率，让判别器D
学会区分真实的因子-收益率对和模型预测的因子-收益率对。
通过对抗训练，G被迫生成在统计上与真实收益率不可区分的预测值，
而不是简单地最小化MSE。
...
\begin{table}[htbp]
\centering
\caption{Factor-GAN 架构参数}
\label{tab:factor_gan_arch}
\begin{tabular}{llr}
\toprule
组件 & 结构 & 参数量 \\
\midrule
\textbf{G}（生成器） & LSTM(256, 2层) $\to$ Linear(64) $\to$ Linear(1) & $\sim$530K \\
\textbf{D}（判别器） & LSTM(128, 2层) $\to$ [h\_last, r] concat $\to$ FC(256$\to$128$\to$1) & $\sim$270K \\
\bottomrule
\end{tabular}
\end{table}

生成器G接收T步因子序列作为输入，输出预测收益率$r_{hat}$；
判别器D接收因子序列和收益率（真实或预测），输出一个WGAN风格的实数分数。

\subsection{损失函数}

Generator的损失函数定义为MSE与对抗项的加权组合：

\begin{align}
    \mathcal{L}_G = \text{MSE}(r_{hat}, y) + \lambda \cdot \mathcal{L}_{adv}
\end{align}

其中 $\mathcal{L}_{adv} = -\mathbb{E}[D(z, r_{hat})]$ 为对抗损失，$\lambda$控制GAN成分的强度：
$\lambda$越大GAN越强，$\lambda$越小越接近纯回归。判别器D采用WGAN-GP框架：

\begin{align}
    \mathcal{L}_D = -\mathbb{E}[D(z, r_{real})] + \mathbb{E}[D(z, r_{hat})] + \lambda_{gp} \cdot \text{GP}
\end{align}

梯度惩罚项GP在$[h_{last}, r]$的联合空间上计算，$\lambda_{gp}=10.0$。

\subsection{数据流}

Factor-GAN使用与路线A V7相同的26维因子管线
（\texttt{build\_windows\_v7.py}），包含19维时序特征和7维截面特征，
序列长度$T=60$，标签为T+1日收益率。
训练方式为滚动窗口（504天训练、63天验证、21天测试，步长21天）。

\subsection{实验阶段1：CNN判别器与$\lambda$扫描}

初始配置使用CNN作为判别器D，Generator Loss中的对抗权重$\lambda$默认设为1.0。
训练配置为504天滑动窗口（约4.8万样本），在验证集上监控IC和方向胜率。

\subsubsection{$\lambda=1.0$（原始配置）}

\begin{itemize}[leftmargin=*]
    \item \textbf{测试结果}：IC=0.0054，ICIR=0.068，方向胜率51.3\%
    \item \textbf{训练现象}：Adv Loss从0.25上升至0.41（D逐渐变强），MSE从8.33降至8.25
    \item \textbf{结论}：GAN存在但未产生有效Alpha，IC几乎为零
\end{itemize}

\subsubsection{$\lambda$递减扫描}

逐步降低$\lambda$以减弱GAN约束，观察IC变化：

\begin{table}[htbp]
\centering
\caption{CNN判别器下$\lambda$扫描结果}
\label{tab:gan_lambda_scan}
\begin{tabular}{cccc}
\toprule
$\lambda$ & Test IC & ICIR & 方向胜率 \\
\midrule
1.0 & 0.005 & 0.068 & 51.3\% \\
0.7 & 0.032 & 0.253 & 45.4\% \\
\textbf{0.6} & \textbf{0.043} & \textbf{0.326} & \textbf{51.6\%} \\
\textbf{0.5} & \textbf{0.050} & \textbf{0.412} & \textbf{51.6\%} \\
0.4 & 0.034 & 0.259 & 51.4\% \\
\bottomrule
\end{tabular}
\end{table}

结果呈现典型的\textbf{倒U型曲线}：$\lambda$过大时GAN过强压制了预测能力，
$\lambda$逐渐降低后IC持续提升，在$\lambda=0.5$处达到峰值，
$\lambda$继续降低后GAN约束太弱，IC反而回落。

\subsection{实验阶段2：LSTM判别器}

在CNN判别器的最优$\lambda$基础上，将判别器从CNN替换为双层LSTM(128)，
使其具备捕获时序依赖的能力。

\begin{itemize}[leftmargin=*]
    \item \textbf{测试结果}：\textbf{IC=0.0603，ICIR=0.6344，方向胜率约51\%}
    \item \textbf{对比}：相比CNN判别器最优IC 0.050，LSTM判别器提升了约20\%
    \item \textbf{结论}：LSTM能更好地利用因子序列的时间依赖信息进行判别
\end{itemize}

\subsection{实验阶段3：扩大训练集}

将训练窗口从504天扩大至756天，样本量从4.8万增加至78.9万：

\begin{itemize}[leftmargin=*]
    \item \textbf{测试结果}：IC=0.0603，ICIR=0.6344（与LSTM D结果一致）
    \item \textbf{验证集稳定性}：val\_ic稳定在0.03--0.06之间，不再出现$\pm$0.05的大幅震荡
    \item \textbf{结论}：此前验证集的波动很大程度源于样本量不足
\end{itemize}

\subsection{关键发现}

\subsubsection{真正带来提升的因素排序}

将各阶段最佳结果汇总：

\begin{table}[htbp]
\centering
\caption{Factor-GAN实验各阶段最佳结果}
\label{tab:gan_progress}
\begin{tabular}{lccc}
\toprule
阶段 & 配置 & Test IC & ICIR \\
\midrule
基线 & 纯MSE回归（无GAN） & 0.045 & --- \\
CNN D $\lambda$=0.5 & CNN判别器 + 最优$\lambda$ & 0.050 & 0.412 \\
LSTM D $\lambda$=0.5 & LSTM判别器 + 最优$\lambda$ & 0.060 & 0.634 \\
大数据 + LSTM D & 756天 + LSTM判别器 & 0.060 & 0.634 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{判别器结构改进（CNN $\to$ LSTM）}：IC提升约+0.010，贡献最大。
          这说明CNN无法有效利用因子序列中的时间依赖信息，
          而LSTM能更好地建模序列模式。
    \item \textbf{增大训练集（504天 $\to$ 756天）}：验证集稳定性大幅提升，
          不再出现大幅震荡，表明此前波动源于样本量不足。
    \item \textbf{调整$\lambda$（1.0 $\to$ 0.5）}：IC提升约+0.015，贡献存在但低于前两项。
          $\lambda$下降带来的收益本质是降低判别器噪声的影响，
          而非GAN本身更有效。
\end{enumerate}

\subsubsection{$\lambda$有效性的重新审视}

$\lambda$递减带来IC提升，但需注意：CNN判别器偏弱，
训练中Adv Loss很快接近零或震荡剧烈，说明D没有提供有价值的梯度。
此时减小$\lambda$反而减少了噪声。因此\textbf{$\lambda$下降带来的收益，
本质是降低弱判别器的负面影响}，而非GAN机制本身的有效性证明。

\subsubsection{数据量的重要性}

扩大训练集后，val\_ic从大幅震荡变为稳定在0.03--0.06。
这表明此前训练中的不稳定性有很大部分来自样本量不足，
而非对抗训练的内在缺陷。

\subsection{实验总结与未解决问题}

从完整的实验链条可以得出以下结论：

\begin{enumerate}[leftmargin=*]
    \item \textbf{GAN框架并非完全无效}：在最优配置（LSTM D, $\lambda=0.5$, 756天）下
          IC=0.0603显著高于纯MSE回归的0.045，表明对抗训练可以提供增量信号。
    \item \textbf{但提升来源需要谨慎归因}：判别器结构改进和增大数据集的贡献
          均大于调整$\lambda$的贡献。$\lambda$扫描找到的最优值是对特定判别器
          架构的补偿，而非GAN框架的普适参数。
    \item \textbf{关键对比实验未完成}：当前最有力的对比应是
          \texttt{LSTM（无GAN）vs LSTM+GAN（$\lambda=0.5$）}，
          在相同数据量和架构下隔离GAN的净贡献。该实验因时间限制未能执行。
    \item \textbf{计算成本仍是瓶颈}：GAN训练速度约为纯MSE的3--5倍，
          在有限的实验周期内限制了充分的调优和对比实验。
\end{enumerate}

尽管Factor-GAN未能在竞赛截止前产出可直接使用的模型，
但上述实验系统地记录了GAN在金融收益率预测上的尝试过程和经验教训，
为后续探索提供了参考基准。
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{吴鸿哲}（PB24030836）：路线B CNN-LSTM模型设计与训练、归一化方案系统性对比、统一评测框架与回测、Factor-GAN补充尝试。
...
train_wu/CNN-LSTM/
  ...
  train.py              训练入口
  my_backtest/          统一回测脚本 (test_correct_backtest.py)
  result/               模型权重、回测结果
\end{verbatim}
...
\begin{verbatim}
train_wu/LSTM-GAN/
  src/factor_gan_lab/      Factor-GAN 核心模块
    models/factor_gan.py    GeneratorLSTM, DiscriminatorLSTM
    training/engine.py      WGAN-GP 训练引擎
    data/dataset.py         数据加载与序列构建
  shared/
    build_windows_v7.py     26维因子窗口构建
    preprocess.py           原始CSV→Parquet合并
  scripts/
