...
防护环节 & 路线A & 路线B \\
\midrule
归一化 & 两阶段归一化：时序特征per-window Z-score，截面特征per-date Z-score（V5修复截面特征零化bug） & 因果Max-Min归一化：每只股票独立，只用截至当日的历史极值计算缩放 \\
...
选择ListMLE而非更常见的MSE回归损失，源于一个关键认知：
...
\begin{tabular}{cclcc}
\toprule
版本 & 核心工作 & 关键决策 & 验证IC & 状态 \\
\midrule
V1 & GRU基线搜索 & 确定$T{=}60,H{=}128,L{=}1$为最优超参 & 0.1042 & 基线 \\
V2 & 多模型对比+特征扩展 & GRU优于TF/MLP；特征从10维扩至22维 & 0.1029 & 早期最优 \\
V3 & 策略回测+止损机制 & Strategy B（动量止损）确立 & — & 策略锚定 \\
V4 & 多窗口集成探索 & $T{=}90$与$T{=}60$高度冗余，放弃多窗口 & — & 放弃 \\
V5 & 截面特征修复 & 两阶段归一化→IC +8.2\% & 0.1114 & \textbf{突破} \\
V6 & 空间注意力 & $d{=}32$ bottleneck是关键设计 & 0.1134 & 架构成功 \\
V7 & T+1标签修正 & 从T+5回归T+1，诚实面对可预测性 & \textbf{0.1062} & \textbf{最优} \\
V8 & 特征扩展+行业嵌入 & 多源数据冲突导致全面退化 & 0.1037 & 失败 \\
V9 & 售卖模型+微调 & 均无增量，前提被证伪 & — & 证伪 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{波动率过滤（Strategy C/A+C）}：全部劣于baseline。根本原因在于
          低波动不等于抗跌——在系统性下跌中，所有股票一起跌，波动率排名不提供选股优势。
          同时波动率过滤也吃掉了GRU原有的alpha信号暴露。
...
\textbf{核心教训}：金融时序中，不同窗口长度的GRU模型学到的是高度相关的排序模式，
...
V5是整个迭代过程中单次IC提升最大的版本，但其核心贡献不是新增了什么，而是\textbf{修复了一个隐藏的P0级bug}。

\subsubsection{缺陷根因：截面特征全军覆没}
...
\subsubsection{V1失败版：缺失bottleneck设计}
...
设计维度 & 原始计划 & V1实现 & 后果 \\
\midrule
Q/K投影维度 & \textbf{$d{=}32$}（bottleneck） & $d{=}128$（=hidden\_size） & 128维全空间计算相似度，无关维度淹没信号 \\
...
\textbf{核心错误}：$d < d_{\text{model}}$是注意力bottleneck设计的核心原则。
...
将Q/K投影到32维bottleneck空间，迫使模型选择对“找邻居”最关键的维度。

\subsubsection{V2修正版：成功的关键}

修正措施：$d_{\text{proj}}=32$ bottleneck + self-exclude
...
三种融合方式全部超越baseline，验证了$d=32$ bottleneck是决定性因素。
...
在T+5上，V6 Spatial的N=5 K=3回测仅比baseline高+0.07\%；
在T+1上，V7 Spatial同等配置的回测比GRU baseline高\textbf{4.5倍}（+15.45\% vs +3.43\%）。
...
\textbf{售卖模型（Phase 0）}：独立训练一个模型专门优化“卖谁”的决策，
使用regime-conditional volatility做卖单排序（熊市卖高波动、牛市卖低波动）。
遍历$\lambda \in [0, 0.5, 1.0, 2.0, 3.0, 5.0, 10.0]$，所有变体的收益均$\leq$baseline。
...
\caption{V9 $\lambda$扫描——任何$\lambda>0$均$\leq$baseline}
\label{fig:v9_lambda}
\end{figure}
...
V1空间注意力（$d{=}128$） & V6 & $d < d_{\text{model}}$是注意力bottleneck核心原则 \\
...
竞赛首日（6月1日）CSI5d为-1.57\%，触发Strategy B减仓信号，
仓位从100\%降至80\%。6月5日再次触发减仓。
10个交易日中风控触发2次（20\%），与历史回测中23.2\%的触发率基本一致。
...
\begin{itemize}[leftmargin=*]
    ...
    \item 累计收益呈现震荡走势，短期（10个交易日）内模型的排序信号面临较大的随机性
...
6月的竞赛期接续了这一衰减趋势——模型的排序能力在regime切换后需要更长的适应期。
...
Wu那边的经历完全印证了这个判断：Max-Min因果归一化和Sigmoid全局归一化的区别，
...
\subsection{如果还有时间，我会尝试什么}

\begin{enumerate}[leftmargin=*]
    \item \textbf{特征级交互机制}。当前GRU把26维特征一视同仁地扔进同一个时序编码器。
          FiLM layer或者cross-feature attention可能让模型学会“成交量放大 + 价格突破”
          这种特征交互模式，突破单因子组合的天花板。
    \item \textbf{路线A和路线B的信号融合}。两条路线的预测分数来自完全不同的方法论
          （ListMLE排序 vs MSE回归），其错误模式可能是互补的。
          简单的IC加权融合就可能产生超越单一模型的信号。
    \item \textbf{滚动再训练}。当前模型用2019--2024的固定窗口训练，
          对2026年的市场regime完全没有适应能力。
          如果每月用最新数据重新训练，模型或许能跟上市场的变化。
    \item \textbf{更多的失败实验}。V8中资金流特征的失败说明“数据源融合”不是简单的列拼接。
          但资金流集中度（mf\_flow\_hhi）的单因子IC高达0.046——
          如果换一种融合方式（如独立的资金流编码器 + 跨模态注意力），
          也许能解锁这部分信号。
    \item \textbf{Transformer with Rotary Position Embedding}。
          路线A的V2中Transformer略逊于GRU，但那是2026年的实验——
          当时的TF用的还是老式sinusoidal position encoding。
          RoPE + FlashAttention的组合在最近的NLP/CV中表现出了远超前代的序列建模能力，
          或许在金融时序上也值得重新评估。
\end{enumerate}
...
姓名 & 学号 & 主要贡献 \\
\midrule
李沐泽 & PB24050961 & 路线A模型设计、V1--V9实验、策略回测、特征工程、报告撰写 \\
吴鸿哲 & PB24030836 & 路线B模型设计、CNN-LSTM训练与回测、归一化方案对比、统一评测 \\
王俊涛 & PB24261889 & 数据处理、模拟交易执行、报告协校 \\
\bottomrule
\end{tabular}
\end{table}
...
train_li/
├── v1-v9/          各版本实验代码
├── v7/models/      最优模型定义（gru\_spatial.py, spatial\_attn.py）
├── v7/checkpoints/ 模型权重
├── report/         报告与图表
