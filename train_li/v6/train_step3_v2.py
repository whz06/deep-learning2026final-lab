"""v6/train_step3_v2.py — Phase 1 spatial attention sweep (S1/S2/S3).

Usage:
  python v6/train_step3_v2.py --fusion concat    # S1
  python v6/train_step3_v2.py --fusion gated     # S2
  python v6/train_step3_v2.py --fusion residual  # S3

Config: d_proj=32, K=10, N_sample=1024 (Phase 1, fast sweep).
"""
...
from v6.models.gru_spatial_v2 import (GRURankerSpatialConcat,
                                       GRURankerSpatialGated,
                                       GRURankerSpatialRes)

FUSION_MAP = {
    "concat":   GRURankerSpatialConcat,
    "gated":    GRURankerSpatialGated,
    "residual": GRURankerSpatialRes,
}
...
def train_one(args, device):
    ...
    name = f"gru_spatial_{fusion_tag}_d{args.d_proj}_K{args.K}_H{args.hidden}_L{args.layers}_D{args.dropout}_lr{args.lr}_N{args.n_sample}"
    ckpt_path = os.path.join(CKPT_DIR, f"{name}.pt")
    csv_path = os.path.join(RESULTS_DIR, "phase1_spatial.csv")

    print(f"\n{'='*60}")
    print(f" Phase 1 Spatial ({fusion_tag.upper()}): {name}")
    print(f"  input_dim={args.input_dim} d_proj={args.d_proj} K={args.K} n_sample={args.n_sample}")
    print(f"  val_every={args.val_every}")
    print(f"{'='*60}")

    print("[step3_v2] Loading dataset ...")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if no_imp >= PATIENCE:
            print(f"  [step3_v2] Early stop @ epoch {epoch}")
            break
    ...
    result = {
        "name": name, "fusion": fusion_tag,
        "input_dim": args.input_dim, "d_proj": args.d_proj, "K": args.K,
    ...
    print(f"\n[step3_v2] DONE: {name} -> IC={best_ic:.4f} @ ep{best_ep} ({elapsed/60:.0f}min)")
    print(f"[step3_v2] Checkpoint -> {ckpt_path}")
...
def main():
    ...
    parser.add_argument("--fusion", type=str, default="concat",
                        choices=["concat", "gated", "residual"],
                        help="Fusion method: concat (S1), gated (S2), residual (S3)")
    ...
    print(f"[step3_v2] device={device}")
